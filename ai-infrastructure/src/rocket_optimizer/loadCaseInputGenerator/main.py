import pandas as pd
from rocket_optimizer.DynamicDatabase import AstosOutput, DatabaseOutput, Masses


def loadCaseInputGenerator():
    ...
    mapping_inert_mass = {
        ...
        "Unaccounted mass": [Masses.mass_unaccounted, AstosOutput.d_n_unaccounted_S1],
        "Stage 1 Unburnt RP1": [Masses.mass_stage1_unburned_RP1, AstosOutput.d_n_stage1_unburned_RP1],
        "Stage 1 Unburnt LOX": [Masses.mass_stage1_unburned_LOX, AstosOutput.d_n_stage1_unburned_LOX],
        ...
        "S2-Propulsion System": [Masses.mass_stage2_propulsion_system, AstosOutput.d_n_stage2_propulsion_system],
        ...
        "Stage 2 Unburnt RP1": [Masses.mass_stage2_unburned_RP1, AstosOutput.d_n_stage2_unburned_RP1],
        "Stage 2 Unburnt LOX": [Masses.mass_stage2_unburned_LOX, AstosOutput.d_n_stage2_unburned_LOX],
        "Stage 2 Tank System Main Propellants/Cryogens": [
            Masses.mass_stage2_tank_system,
            AstosOutput.d_n_stage2_tank_system,
        ],
        "Stage 3 Dry mass": [Masses.mass_stage3_dry, AstosOutput.d_n_S3_dry],
        "Stage 3 Propellant": [Masses.mass_stage3_prop, AstosOutput.d_n_S3_prop],
        "Payload": [Masses.mass_payload, AstosOutput.d_n_payload],
        "Fairing": [Masses.mass_fairing, AstosOutput.d_n_fairing],
    }
    ...
    for idx, row in df_Printout_Locations.iterrows():
        column_d_value = row[column_d_name_Printout_Locations]
        if column_d_value in mapping_Printout_Locations:
            df_Printout_Locations.at[idx, df_Printout_Locations.columns[1]] = mapping_Printout_Locations[column_d_value]

    # Load the original sheets that are not updated
    sheets_to_preserve = {
        sheet: excel_data.parse(sheet)
        for sheet in excel_data.sheet_names
        if sheet not in ["General", "Inert Mass", "Printout Locations"]
    }
